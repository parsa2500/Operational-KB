...
export async function extractRoslyn(root: string): Promise<ExtractedEvidence[]> {
  ...
  const child = spawn('dotnet', ['run', '--project', project, '--configuration', 'Release', '--no-launch-profile', '--', root], { stdio: ['ignore', 'pipe', 'pipe'] });
  let stdout = ''; let stderr = '';
  child.stdout.on('data', (chunk) => { stdout += chunk.toString(); });
  child.stderr.on('data', (chunk) => { stderr += chunk.toString(); });
  const [result] = await once(child, 'close') as [number];
  if (result !== 0) throw new Error(`Roslyn analyzer failed (${result}): ${stderr.slice(-4000)}`);
