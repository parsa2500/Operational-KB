...
const heuristicKind = (p: string, l: string) => /\.(tsx|jsx)$/.test(p) && /(button|menu|route|path|label)/i.test(l) ? 'ui' : /(fetch|axios|httpclient|\[Http(Get|Post|Put|Delete|Patch))/i.test(l) ? 'api' : /(authorize|permission|role)/i.test(l) ? 'permission' : /(status|state|approve|confirm|تایید)/i.test(l) ? 'state' : /\.md$/.test(p) ? 'documentation' : 'code';
...
export async function ingest(root: string) {
  ...
  const files = await fg(['**/*.{ts,tsx,js,jsx,cs,md,json,yaml,yml}'], { cwd: absoluteRoot, ignore: ['**/.git/**','**/node_modules/**','**/dist/**','**/build/**','**/bin/**','**/obj/**','**/coverage/**','**/*.lock','**/.env*'], onlyFiles: true, followSymbolicLinks: false });
  ...
  const existing = await db.query("SELECT id,created_at FROM snapshots WHERE revision=$1 AND status='active' LIMIT 1", [revision]);
  ...
  try {
    ...
    for (const file of loaded) {
      if (/\.md$/.test(file.path) && file.content.trim()) semantic.push({ source: 'typescript', path: file.path, lineStart: 1, lineEnd: file.content.split(/\r?\n/).length, kind: 'documentation', title: file.path, content: file.content.slice(0, 5000), metadata: {} });
      const lines = file.content.split(/\r?\n/);
      for (let i = 0; i < lines.length; i++) {
        if (!interesting.test(lines[i])) continue;
        const a = Math.max(0, i - 2), b = Math.min(lines.length, i + 3), text = lines.slice(a, b).join('\n').trim();
        if (text) semantic.push({ source: 'typescript', path: file.path, lineStart: a + 1, lineEnd: b, kind: heuristicKind(file.path, lines[i]), title: lines[i].trim().slice(0, 180), content: text, metadata: { fallback: true } });
      }
    }
    ...
    for (const evidence of semantic) {
      const normalized = { ...evidence, path: evidence.path.replaceAll('\\','/').replace(/^\.\//,'') };
    ...
    await client.query("UPDATE snapshots SET status='staging' WHERE status='active'");
    await client.query("UPDATE snapshots SET status='active' WHERE id=$1", [snapshot]);
