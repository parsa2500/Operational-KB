...
export function queryTerms(question: string) {
  const base = question.toLowerCase().split(/[^\p{L}\p{N}_./-]+/u).filter((x) => x.length > 1 && !ignored.has(x));
...
export async function retrieve(question: string, limit = 14): Promise<Evidence[]> {
  ...
  const query = terms.map((term) => term.replace(/[':&|!()]/g, '')).filter(Boolean).join(' | ');
  const like = `%${terms.slice(0, 8).join('%')}%`;
  const result = await db.query(`
    ...
    ORDER BY score DESC, e.path, e.line_start LIMIT $3`, [query, terms.slice(0, 12).map((term) => `%${term.toLowerCase()}%`), limit]);
  if (result.rows.length) return result.rows;
  const fallback = await db.query(`SELECT e.id,e.revision,e.path,e.line_start AS "lineStart",e.line_end AS "lineEnd",e.kind,e.title,e.content,e.metadata FROM evidence e JOIN snapshots s ON s.id=e.snapshot_id WHERE s.status='active' AND lower(e.content) LIKE $1 LIMIT $2`, [like, limit]);
