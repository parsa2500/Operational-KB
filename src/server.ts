...
app.post('/api/query', async (request, reply) => {
  const parsed = z.object({ question: z.string().min(3).max(2000), limit: z.number().int().min(1).max(30).default(config.RETRIEVAL_LIMIT), history: z.array(messageSchema).max(10).default([]) }).safeParse(request.body);
  if (!parsed.success) return reply.code(400).send({ error: parsed.error.flatten() });
  try {
    const evidence = await retrieve(parsed.data.question, parsed.data.limit);
    return await answer(parsed.data.question, evidence, parsed.data.history);
  } catch (error) {
    request.log.error(error);
    return reply.code(502).send({ error: error instanceof Error ? error.message : 'Query failed' });
  }
});
